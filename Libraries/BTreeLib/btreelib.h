...
struct Node{
		...
		int *key;								//pointer to an array of keys in the node
	}*root;
...
//INSERTION
void btreeInsert(int);							//inserts the given key into the B-tree using btreeInsertNonFull()
void btreeInsertNonFull(struct Node *,int);		//inserts the given key into a non full node in the B-tree
